...
def apply_quantize_preproc(X, n_bins=256):
    def scale(x):
        # scale to 0, 1
        x = x.astype("float64")
        x = (x - x.min()) / (x.max() - x.min())
...
def run_blizzard_example():
    sz = '1000'
    bliz = Blizzard_dataset(wav_folder_path='./blizzard/wavn_{}'.format(sz),
...
def run_blizzard_example_part():
    sz = '1000_8'
    bliz_ds = Blizzard_dataset(wav_folder_path='./blizzard/wavn_{}'.format(sz),
                               prompt_path='./blizzard/prompts_{}.txt'.format(sz),
                               preproc_fn=wav_to_qbins_frames,
                               frame_size=1,
                               thread_cnt=1,
                               minibatch_size=20,
                               fraction_range=[0, 800])
    start = time.time()
    itr = 1
    for i in range(1):
        bliz = iter(Blizzard_dataset_adapter(bliz_ds, cut_len=256, overlap=8))
        try:
            while True:
                X_mb, X_mb_mask, c_mb, c_mb_mask, reset = next(bliz)
                itr += 1
                print('empty cells ratio', np.sum(X_mb_mask),
                      np.prod(X_mb_mask.shape), '=', 1.*np.sum(X_mb_mask)/np.prod(X_mb_mask.shape))
                if reset:
                    print('sum', X_mb.sum(), i)
        except StopIteration:
            pass
    stop = time.time()
    tot = stop - start
